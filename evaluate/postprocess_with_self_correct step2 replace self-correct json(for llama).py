...
file_path  = r'./correct_relation_llama70b'
with open(file_path, 'r', encoding='utf-8') as file:
    correct_relation_llama70b = json.load(file)
            ...
            if all(isinstance(item, str) for item in relationship):
                ...
                for relation in relationship[1:]:
                    for sub_relation in re.split(r',|、|，|\.', relation):
                        ...
                        if sub_relation not in combined_list:
                            sub_relation = correct_relation_llama70b.get(sub_relation, '')
                            all_relation.append(sub_relation)
                        sub_relation = sub_relation.replace(related_character,'x').replace(character,'x')
                        if ' of' in  sub_relation and ' of x' not in sub_relation and sub_relation != "Perpetrator of X's family":
...
if __name__ == '__main__':
    ...
    language = 'chinese'
    folder_path = r'../data/'+language+'/llama3_70b_v2'

    if folder_path.split('/')[-1] == 'llama-70b_v2':
        origin = 'llama-70b_v2'
        replace = 'llama-70b_v3'
    elif folder_path.split(os.path.sep)[-1] == 'llama3_70b_v2':
        origin = 'llama3_70b_v2'
        replace = 'llama3_70b_v3'
    else:
        print("wrong")
        exit()
